...
// 判断租先节点是否含有 template-jump target=swan|wx，如果含有，则不收集该ast tag
function checkScopedTemplateIsH5(ast) {
    let parent = ast.parent;
    if (parent) {
        if (parent.tag === 'template-jump' && parent.attrsMap && parent.attrsMap.target !== 'h5') {
            return false;
        }
        else {
...
module.exports = function (ast, options) {
    ...
    if (
        tag === customTemplate
        && ast.attrsMap.target === (process.env.JUMP_ENV_TARGET || 'h5')
    ) {
        tag = 'template';
