...
export function onSocketOpen(cb) {
    // JumpSocket[0].addEventListener('open', event => cb(event) });
    JumpSocket[0].onopen = cb;
}

export function onSocketError(cb) {
    // JumpSocket[0].addEventListener('error', event => cb(event) });
    JumpSocket[0].onerror = cb;
}
...
export function closeSocket(options) {
    ...
    let err = null;
    try {
        JumpSocket[0].close(code, reason);
    } catch (e) {
        err = e;
    }
