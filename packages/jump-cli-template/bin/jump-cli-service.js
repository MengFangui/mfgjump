...
// const dest = process.env.JUMP_CLI_DEST || './path-h5';
// const context = path.resolve(process.cwd(), dest);
// default JUMP_CLI_OPTIONS when run jump-cli-service directly
// process.env.JUMP_CLI_TARGET = process.env.JUMP_CLI_TARGET || 'h5';
// process.env.JUMP_CLI_OPTIONS = process.env.JUMP_CLI_OPTIONS || JSON.stringify({
//     target: 'h5'
// });
...
(async () => {
    ...
    if (!process.env.VUE_CLI_SERVICE_CONFIG_PATH) {
        const env = process.env.JUMP_CLI_ENV;
    ...
    try {
        pwaSupport = JSON.parse(process.env.JUMP_PWA);
    }
    catch (e) {}
