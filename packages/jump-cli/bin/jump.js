...
checkNodeVersion(requiredVersion, 'jump-cli');
...
program
    ...
    .action(async (name, cmd) => {
        ...
        files = await globby(['**/*'], {cwd: basePath + '/jump', deep: false, onlyFiles: false});
        for (const rawPath of files) {
            await fs.move(basePath + '/jump' + '/' + rawPath, basePath + '/' + rawPath);
        }
        fs.remove(basePath + '/jump');
    });
...
program
    ...
    .option('--h5skip <process>', 'Skip h5 compile process (jump | vue)')
    .action(cmd => {
        const options = cleanArgs(cmd);
        const buildPath = path.resolve(__dirname, './jump-build.js');
...
program
    ...
    .action(cmd => {
        const options = cleanArgs(cmd);
        const buildPath = path.resolve(__dirname, './jump-serve.js');
...
program
    .command('update')
    .description('update all jump dependences')
    .option('-r, --registry <url>', 'Use specified npm registry when installing dependencies (only for npm)')
...
program
    .command('info')
    .description('Diagnostics Jump env info')
    .action(cmd => {
