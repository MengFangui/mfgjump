...
const JUMP_ENV = process.env.JUMP_ENV_TARGET || 'h5';
...
exports.compileScript = async function (content, options = {}) {
    ...
    content = content.replace(
        /process\.env\.JUMP_ENV/g,
        JSON.stringify(JUMP_ENV)
    ).replace(
        /process\.env\.NODE_ENV/g,
        JSON.stringify(process.env.NODE_ENV || 'development')
    );
    const useAOP = jump.useAOP === undefined ? true : jump.useAOP;
...
//     // 判断当前JUMP_ENV模式下是否有对应的页面数组
//     let envPages = [];
//     if (config && config['pages-' + JUMP_ENV]) {
//         envPages = config['pages-' + JUMP_ENV];
//     }
//     const routes = pages.concat(subPages, envPages);

//     const mode = jump && jump.mode ? jump.mode : 'history';
...
//             window,
//             jump: {
//                 navigationBarHomeColor: jump.navigationBarHomeColor === undefined
//                     ? 'dark'
//                     : jump.navigationBarHomeColor,
//                 showNavigationBorder: !!jump.showNavigationBorder,
//                 useTransition: jump.useTransition === undefined ? true : jump.useTransition,
//                 homePage: `/${list.length > 0 ? list[0].pagePath : pages[0]}`,
//                 supportPWA: !!jump.supportPWA
//             },
//             componentSet,
//             pagesInfo
//         })]
