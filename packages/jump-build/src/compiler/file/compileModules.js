...
const modules = {
    ...
    'vuex': {
        needCompile: true,
        path: './jump_modules/vuex/index.js'
    }
};
...
function getModulePath(modulePath, modules) {
    ...
    modulePath = './jump_modules' + entry.slice(entry.lastIndexOf('node_modules')).replace('node_modules', '');
    return modulePath;
}
...
function getUIModules(components, target) {
    ...
    Object.keys(components).forEach(key => {
        ...
        if (mod[0] !== '.') {
            ...
            const path = `./jump_modules/${name}/dist/${target}`;
...
async function compileUIModules(uiModules, destPath) {
    for (const key of Object.keys(uiModules)) {
        ...
        const coreEntry = require.resolve(realName + '/jump-core', {
            paths: [process.cwd()]
        });
        const entry = coreEntry.replace('jump-core/index.js', '');
        const dest = path.resolve(destPath, modPath);
        const coreDestPath = path.resolve(destPath, 'jump-core');
        const uiCoreDestPath = path.resolve(dest, 'jump-core');
