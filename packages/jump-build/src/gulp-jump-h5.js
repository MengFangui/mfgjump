...
// const customTemplate = 'template-jump';

// 常量
const PLUGIN_NAME = 'gulp-jump';
...
async function compile(file, opt) {
    ...
    const jumpConfig = options._config;
    ...
    scriptRet = await scriptCompiler(script, {
        ...
        path: fPath + '.vue',
        dest: jumpConfig.dest,
        jump: jumpConfig.h5 || {}
    });
    // use blockConfig first
    config = mpConfig ? mpConfig : (scriptRet && scriptRet.config);
    // prefer appConfig in jumpConfig
    if (isApp) {
        const appConfig = jumpConfig.appConfig && jumpConfig.appConfig.config;
    ...
    if (!isApp) {
        ...
        const {componentsInUsed} = await templateCompiler(template, {
            ...
            target: process.env.JUMP_ENV_TARGET || 'h5'
        });
    ...
    if (isApp) {
        ...
        // fs.writeFileSync(options.dest + '/main.js', `import main from './jump-core/template/main'`);
        // 处理 globalApi
...
//         !item.attrs || (!item.attrs.target || item.attrs.target === (process.env.JUMP_ENV_TARGET || 'h5')));
