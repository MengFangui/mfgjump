...
function getProjectConfig(target) {
    let projectConfig = {};
    const configPath = path.resolve(process.cwd(), './jump.config.js');
...
function formatConfig(options) {
    ...
    const targetEnv = process.env.JUMP_ENV_TARGET || target;
 ...
 * getConfig
 *
 * @param {jump.options} options options
 * @return {jump.config}
 */
function getConfig(options) {
    if (!options && process.env.JUMP_CLI_OPTIONS) {
        try {
            options = JSON.parse(process.env.JUMP_CLI_OPTIONS);
        }
        catch (e) {}
    }
    if (!options) {
